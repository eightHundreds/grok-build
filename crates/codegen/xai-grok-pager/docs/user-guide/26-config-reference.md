...
| `model.<id>.keychain_account` | `string` | `yes` | `user` | OS keychain account/username. Enough on its own; looks up service `grok` unless `keychain_service` is set. Missing item is treated like an empty `env_key`. |
| `model.<id>.keychain_service` | `string` | `yes` | `user` | Optional OS keychain service override. Default `grok`. Not recommended to change. Resolved after `api_key`/`env_key`, before the session token. Never logged. |
...
| `ui.official_usage` | `boolean` | `yes` | `user` | Show official xAI account quota / billing chrome (`/usage` **Usage limit**, credit warnings, billing fetches). Default false; must set to show. Also `GROK_OFFICIAL_USAGE`. Session token totals and `grok usage <session-id>` stay available. |
