...
| `model.<id>.keychain_account` | `string` | `yes` | `user` | 与 `keychain_service` 配对的系统钥匙串 account/用户名。两者都必须设置。项缺失时等同于空的 `env_key`。 |
| `model.<id>.keychain_service` | `string` | `yes` | `user` | 系统钥匙串 service 名（macOS 钥匙串、Windows 凭据管理器）。与 `keychain_account` 成对使用。在 `api_key`/`env_key` 之后、会话 token 之前解析。密钥不会被记录。 |
