...
凭据解析：`api_key` > `env_key` > 钥匙串（`keychain_account`；`keychain_service` 默认为 `grok`，不建议改）> 已登录会话 token > `XAI_API_KEY`。`query_params`、`env_http_headers` 和 `extra_body` 见 [自定义模型](11-custom-models.md#request-query-parameters)，限制工具子进程继承哪些环境变量的 `[shell_environment_policy]` 见 [沙箱模式](18-sandbox.md#shell-environment-policy)。
